...
transcript::transcript() : md_(default_digest()), ctx_(create_digest_context(), EVP_MD_CTX_free)
{
    ...
    {
        LOG_ERROR("{} stage transcript_init error digest_init_failed", relay::log_event::kHandshake);
        valid_ = false;
        return;
    }
    valid_ = true;
...
void transcript::set_protocol_hash(const EVP_MD* new_md)
{
    ...
    if (new_md == nullptr)
    {
        LOG_ERROR("{} stage set_protocol_hash error null_digest", relay::log_event::kHandshake);
        valid_ = false;
        return;
    }
    ...
    if (new_ctx == nullptr)
    {
        LOG_ERROR("{} stage set_protocol_hash error create_digest_context_failed", relay::log_event::kHandshake);
        valid_ = false;
        return;
    }
    if (EVP_DigestInit_ex(new_ctx.get(), new_md, nullptr) != 1)
    {
        LOG_ERROR("{} stage set_protocol_hash error digest_reinit_failed", relay::log_event::kHandshake);
        valid_ = false;
        return;
    }
    if (!buffer_.empty() && EVP_DigestUpdate(new_ctx.get(), buffer_.data(), buffer_.size()) != 1)
    {
        LOG_ERROR("{} stage set_protocol_hash buffer_size {} error digest_replay_failed", relay::log_event::kHandshake, buffer_.size());
        valid_ = false;
        return;
    }
    md_ = new_md;
...
void transcript::update(const std::vector<uint8_t>& data)
{
    ...
    {
        LOG_ERROR("{} stage transcript_update size {} error digest_update_failed", relay::log_event::kHandshake, data.size());
        valid_ = false;
    }
}
...
std::vector<uint8_t> transcript::finish() const
{
    ...
    {
        LOG_ERROR("{} stage transcript_finish error copy_digest_context_failed", relay::log_event::kHandshake);
        return {};
    }
    ...
    if (hash_len <= 0)
    {
        LOG_ERROR("{} stage transcript_finish error invalid_hash_len {}", relay::log_event::kHandshake, hash_len);
        return {};
    }
    ...
    {
        LOG_ERROR("{} stage transcript_finish expected_len {} actual_len {} error digest_final_failed", relay::log_event::kHandshake, h.size(), l);
        return {};
    }
    return h;
}
