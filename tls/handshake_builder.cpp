...
void push_bytes(std::vector<uint8_t>& buf, const std::vector<uint8_t>& data) { buf.insert(buf.end(), data.begin(), data.end()); }

void push_vector_u8(std::vector<uint8_t>& buf, const std::vector<uint8_t>& data)
{
    buf.push_back(static_cast<uint8_t>(data.size()));
    push_bytes(buf, data);
}

void push_vector_u16(std::vector<uint8_t>& buf, const std::vector<uint8_t>& data)
{
...
std::vector<uint8_t> write_record_header(uint8_t record_type, uint16_t length)
{
    ...
    header.push_back(static_cast<uint8_t>((tls::consts::kVer12 >> 8) & 0xff));
    header.push_back(static_cast<uint8_t>(tls::consts::kVer12 & 0xff));
                                            ...
                                            const std::span<const uint16_t> extension_order)
{
    ...
    std::vector<uint8_t> supported_versions_ext;
    ...
    push_u16(hello, static_cast<uint16_t>(extensions.size()));
                                                    ...
                                                    const std::optional<uint16_t> padding_len)
{
    ...
    if (!alpn.empty())
    {
        ...
        push_vector_u8(proto, std::vector<uint8_t>(alpn.begin(), alpn.end()));
        std::vector<uint8_t> ext;
        push_vector_u16(ext, proto);
        push_u16(alpn_ext, static_cast<uint16_t>(ext.size()));
        push_bytes(alpn_ext, ext);
    }

    std::vector<uint8_t> padding_ext;
    if (include_padding)
    {
        constexpr std::size_t kMaxExtensionsLen = 65535;
        ...
        uint16_t resolved_padding_len = 0;
        if (padding_len.has_value())
        {
            resolved_padding_len = static_cast<uint16_t>(std::min<std::size_t>(static_cast<std::size_t>(*padding_len), padding_budget));
        }
        else
        {
            static thread_local std::mt19937 gen(std::random_device{}());
            std::uniform_int_distribution<uint16_t> dist(10, 100);
            resolved_padding_len = static_cast<uint16_t>(std::min<std::size_t>(static_cast<std::size_t>(dist(gen)), padding_budget));
        }
        ...
        if (padding_budget > 0)
        {
            ...
            for (uint16_t i = 0; i < resolved_padding_len; ++i)
            {
    ...
    push_u16(msg, static_cast<uint16_t>(extensions.size()));
...
std::vector<uint8_t> construct_certificate(const std::span<const std::vector<uint8_t>> cert_chain)
{
    std::vector<uint8_t> msg;
    msg.push_back(0x0b);
    ...
    {
        push_u24(list, static_cast<uint32_t>(cert_der.size()));
        push_bytes(list, cert_der);
        push_u16(list, 0x0000);
    }
    push_u24(body, static_cast<uint32_t>(list.size()));
    push_bytes(body, list);
    push_u24(msg, static_cast<uint32_t>(body.size()));
    push_bytes(msg, body);
    return msg;
}
...
{
    return construct_certificate(std::span<const std::vector<uint8_t>>(&cert_der, 1));
}
...
std::vector<uint8_t> construct_certificate_verify(EVP_PKEY* signing_key, const std::vector<uint8_t>& handshake_hash)
{
    ...
    if (sig_len > std::numeric_limits<uint16_t>::max())
    {
        return {};
    }
    ...
    push_u16(body, static_cast<uint16_t>(sig_len));
...
std::vector<uint8_t> construct_finished(const std::vector<uint8_t>& verify_data)
{
    std::vector<uint8_t> msg;
    msg.push_back(0x14);
    push_u24(msg, static_cast<uint32_t>(verify_data.size()));
