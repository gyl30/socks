...
start_in_ns "$ns_wan" "$tmp_dir/reality-server.stdout.log" env SOCKS_CONFIG_DIR="$repo_root/config" "$binary" -c "$tmp_dir/server.json"
...
start_in_ns "$ns_mid" "$tmp_dir/tproxy-client.stdout.log" env SOCKS_CONFIG_DIR="$repo_root/config" "$binary" -c "$tmp_dir/client.json"
...
wait_log_pattern_since "$client_log" "$start_line" "selecting backend route direct" 5 || {
...
run_step "tcp proxy slow success" \
    ...
    --read-timeout 15 \
    --expect-substring "socks5-slow-http-server"
wait_log_pattern_since "$client_log" "$start_line" "selecting backend route proxy" 5 || {
...
run_expect_failure "tcp direct connect timeout" \
    ...
    --path "/" \
    --read-timeout 8
wait_log_pattern_since "$client_log" "$start_line" "backend connect failed target ${direct_tcp_drop_ip}:${direct_tcp_drop_port} route direct" 8 || {
...
run_expect_failure "tcp proxy connect timeout" \
    ...
    --path "/" \
    --read-timeout 8
if ! wait_log_pattern_since "$client_log" "$start_line_client" "stage=wait_ack target=${proxy_tcp_drop_ip}:${proxy_tcp_drop_port} remote_rep=" 8 \
    && ! wait_log_pattern_since "$server_log" "$start_line_server" "${proxy_tcp_drop_ip}:${proxy_tcp_drop_port}" 8; then
...
print_log_count "proxy_connect_fail_ack" "$client_log" "stage=wait_ack target=${proxy_tcp_drop_ip}:${proxy_tcp_drop_port} remote_rep="
