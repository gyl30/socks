...
resolved_outbound resolve_outbound(const config& cfg, const std::string_view outbound_tag)
{
    resolved_outbound result;
    const auto* outbound = find_outbound_entry(cfg, outbound_tag);
    if (outbound == nullptr)
    {
        return result;
    }
    result.entry = outbound;
    result.outbound_class = config_type::classify_outbound_type(outbound->type);
    return result;
}

std::shared_ptr<tcp_outbound_stream> create_tcp_outbound_for_resolved(const boost::asio::any_io_executor& executor,
...
{
    if (outbound.outbound_class == config_type::outbound_class::kDirect)
    {
        return make_direct_tcp_outbound_stream(executor, conn_id, trace_id, cfg, connect_mark);
    }
    if (outbound.outbound_class == config_type::outbound_class::kProxy)
    {
                                                                                      ...
                                                                                      const uint32_t timeout_sec)
{
    ...
    if (outbound.outbound_class == config_type::outbound_class::kBlock)
    {
        udp_proxy_outbound_connect_result result;
