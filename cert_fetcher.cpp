...
const char* cert_log_sni(const fetch_context& ctx)
{
    if (!ctx.sni.empty())
    {
        return ctx.sni.c_str();
    }
    return "unknown";
}
...
bool validate_server_hello_body(fetch_context& ctx, const std::vector<uint8_t>& server_hello_body)
{
