                                                 ...
                                                 const config& cfg)
{
    ...
    const auto outbound = resolve_outbound(cfg, result.decision.outbound_tag);
    ...
    result.outbound = create_tcp_outbound_for_resolved(
        executor, request.conn_id, request.trace_id, cfg, result.decision.outbound_tag, outbound, connect_mark);
    if (result.outbound == nullptr && outbound.outbound_class != config_type::outbound_class::kBlock)
    {
