...
{
   public:
    LocalSession(boost::asio::ip::tcp::socket socket, std::shared_ptr<MuxTunnel> tunnel)
        : socket_(std::move(socket)), tunnel_(tunnel), udp_socket_(socket_.get_executor())
    {
        boost::system::error_code ec;
        socket_.set_option(boost::asio::ip::tcp::no_delay(true), ec);
        if (ec)
            LOG_WARN("LocalSession set nodelay error: {}", ec.message());
    }
   ...
   private:
    ...
    boost::asio::awaitable<std::tuple<boost::system::error_code, std::uint8_t, std::string, std::uint16_t>> read_request()
    {
        std::uint8_t head[4];
        ...
        {
            LOG_WARN("read_request unsupported atyp: {}", head[3]);
    ...
    boost::asio::awaitable<void> connect_remote(std::uint8_t cmd, const std::string& host, std::uint16_t port)
    {
        ...
        mux::ack_payload ack;
        if (!mux::ack_payload::decode(ack_buf.data(), ack_buf.size(), ack))
        {
    ...
    boost::asio::awaitable<void> setup_udp_associate(const std::string& client_host, std::uint16_t client_port)
    {
        boost::system::error_code ec;
    ...
    boost::asio::awaitable<void> tcp_keepalive()
    {
        char buf[1];
        while (true)
        {
    ...
    boost::asio::awaitable<void> transfer_udp_to_stream(std::shared_ptr<MuxStream> stream)
    {
    ...
    boost::asio::awaitable<void> transfer_stream_to_udp(std::shared_ptr<MuxStream> stream)
    {
        while (true)
        {
...
{
   public:
    LocalClient(io_context_pool& pool, const std::string& remote_host, const std::string& remote_port, std::uint16_t local_port)
