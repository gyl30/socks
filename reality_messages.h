...
class chrome_client_hello_builder
{
   public:
    ...
    [[nodiscard]] static std::vector<uint8_t> build_alpn()
    {
        ...
        std::vector<uint8_t> protos = {2, 'h', '2', 8, 'h', 't', 't', 'p', '/', '1', '.', '1'};
    ...
    [[nodiscard]] static std::vector<uint8_t> build_signature_algorithms()
    {
        ...
        for (auto a : algs) message_builder::push_u16(ext, a);
        return ext;
    }
...
{
    uint16_t g_cipher = crypto_util::get_random_grease();
    uint16_t g_ext1 = crypto_util::get_random_grease();
    uint16_t g_ext2 = crypto_util::get_random_grease();
    uint16_t g_group = crypto_util::get_random_grease();
    uint16_t g_ver = crypto_util::get_random_grease();
    ...
    std::vector<uint16_t> suites = {g_cipher, 0x1301, 0x1302, 0x1303, 0xc02b, 0xc02f};
    message_builder::push_u16(hello, static_cast<uint16_t>(suites.size() * 2));
    for (auto s : suites) message_builder::push_u16(hello, s);
                                                   ...
                                                   const std::vector<uint8_t> &server_public_key)
{
    ...
    uint16_t ext_len = static_cast<uint16_t>(2 + 2 + server_public_key.size());
...
inline std::vector<uint8_t> extract_server_public_key(const std::vector<uint8_t> &server_hello)
{
    ...
    if (pos + 2 > server_hello.size())
        return {};
    uint16_t ext_len = static_cast<uint16_t>((server_hello[pos] << 8) | server_hello[pos + 1]);
    pos += 2;

    size_t end = pos + ext_len;
    while (pos + 4 <= end)
    {
        uint16_t etype = static_cast<uint16_t>((server_hello[pos] << 8) | server_hello[pos + 1]);
        uint16_t elen = static_cast<uint16_t>((server_hello[pos + 2] << 8) | server_hello[pos + 3]);
        pos += 4;
        if (etype == 51 && elen >= 4)
        {
            uint16_t klen = static_cast<uint16_t>((server_hello[pos + 2] << 8) | server_hello[pos + 3]);
            if (klen == 32)
