...
boost::asio::awaitable<bool> tun_udp_session::run()
{
    ...
    record_udp_trace_stage(make_transparent_udp_trace_event(trace_id_,
                                                            ...
                                                            match_type_,
                                                            match_value_),
                           trace_stage::kRouteDecideDone,
                           (route_ == route_type::kBlock) ? trace_result::kFail : trace_result::kOk);
...
boost::asio::awaitable<void> tun_udp_session::direct_to_client()
{
    ...
    co_await relay_connected_udp_socket_replies(
        ...
        [this](const boost::system::error_code& ec)
        {
            LOG_WARN("{} trace {:016x} conn {} client {}:{} target {}:{} read tun direct udp reply failed {}",
                     log_event::kRelay,
                     trace_id_,
                     conn_id_,
                     client_endpoint_.address().to_string(),
...
boost::asio::awaitable<void> tun_udp_session::proxy_to_client()
{
    ...
    co_await relay_proxy_outbound_replies(
        ...
        [this](const boost::system::error_code& ec)
        {
            LOG_WARN("{} trace {:016x} conn {} client {}:{} target {}:{} read tun proxy udp datagram failed {}",
                     log_event::kRoute,
                     trace_id_,
                     conn_id_,
                     client_endpoint_.address().to_string(),
...
void tun_udp_session::mark_transport_error(const boost::system::error_code& ec)
{
