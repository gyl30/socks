    ...
    static ClientHelloData parse(const std::vector<uint8_t>& buf)
    {
        ...
        while (p + 4 <= ext_end)
        {
            ...
            if (etype == 0x0033 && elen >= 2)
            {
                ...
                while (sp + 4 <= se)
                {
                    uint16_t grp = (sp[0] << 8) | sp[1];
                    uint16_t klen = (sp[2] << 8) | sp[3];
                    sp += 4;
                    if (sp + klen > se)
                        break;
...
class remote_session : public IMuxStream, public std::enable_shared_from_this<remote_session>
{
   public:
    remote_session(std::shared_ptr<MuxConnection> connection, uint32_t id, const boost::asio::any_io_executor& ex)
...
class remote_server
{
   ...
   private:
    ...
    boost::asio::awaitable<void> handle(std::shared_ptr<boost::asio::ip::tcp::socket> s)
    {
        ...
        auto info = CHParser::parse(ch_msg);
        ...
        {
            LOG_ERROR("RemoteServer invalid client finished.");
            co_return;
        }
