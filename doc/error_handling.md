...
本项目当前采用“`std::expected` 为主，`boost::asio::awaitable<boost::system::error_code>` 作为异步边界接口”的混合模式。

## 错误处理模式

### 1. `std::expected<T, boost::system::error_code>`（同步与业务层）
...
不返回值时使用 `std::expected<void, boost::system::error_code>`。

### 2. `boost::asio::awaitable<boost::system::error_code>`（异步 I/O 边界）

适用于：socket 读写、握手分段发送/接收等“是否成功”语义的协程边界接口。

```cpp
boost::system::error_code ec = co_await conn->send_async(stream_id, cmd, payload);
if (ec)
{
...
### 3. `boost::asio::awaitable<std::expected<T, boost::system::error_code>>`（异步且需要返回值）
...
2. 错误通过 `std::expected`、`std::optional`、`boost::system::error_code` 或显式状态返回。
3. 失败路径必须可测试、可日志观测，不依赖异常捕获。

## 规范要点

1. 新增业务接口优先使用 `std::expected`。
2. 新增异步 I/O 边界接口允许使用 `boost::asio::awaitable<boost::system::error_code>`，禁止回退到 `boost::system::error_code&` 输出参数。
...
2. 新代码保持“业务 `expected`、边界 `awaitable<boost::system::error_code>`”。
...
2. 禁止新增 `boost::system::error_code&` 输出参数风格接口。
