...
class cert_manager
{
   public:
    ...
    void set_certificate(const std::string& sni, std::vector<uint8_t> cert_msg, server_fingerprint fp)
    {
        const std::scoped_lock lock(mutex_);
        ...
        LOG_INFO("cert manager cached certificate for sni {} size {} alpn '{}' cipher 0x{:04x}",
                 sni,
