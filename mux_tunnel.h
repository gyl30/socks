...
class mux_stream : public IMuxStream, public std::enable_shared_from_this<mux_stream>
{
   public:
    mux_stream(std::uint32_t id, const std::shared_ptr<MuxConnection>& connection, const boost::asio::any_io_executor& ex)
    ...
    boost::asio::awaitable<std::tuple<boost::system::error_code, std::vector<std::uint8_t>>> async_read_some()
    {
...
{
   public:
    explicit mux_tunnel_impl(StreamLayer socket, RealityEngine engine)
        : connection_(std::make_shared<MuxConnection>(std::move(socket), std::move(engine), streams_, mutex_))
    {
    }

    std::shared_ptr<MuxConnection> get_connection() const { return connection_; }

    void register_stream(uint32_t id, std::shared_ptr<IMuxStream> stream)
    {
   ...
   private:
    MuxConnection::StreamMap streams_;
    std::mutex mutex_;
    std::shared_ptr<MuxConnection> connection_;
