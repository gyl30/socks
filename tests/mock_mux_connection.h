...
public:
    // Create a dummy socket for the base constructor. 
    // We don't use it because we're mocking the methods.
    MockMuxConnection(asio::io_context& ctx) 
        : mux_connection(asio::ip::tcp::socket(ctx), reality_engine{{}, {}, {}, {}}, true, 0) {}
    ...
    // send_async is more complex because it returns an awaitable.
    // We can use a helper to mock awaitables or just use a simple implementation.
