...
void test_ecdh_set(const std::string& label, const std::vector<uint8_t>& priv1, const std::vector<uint8_t>& pub1, 
                   const std::vector<uint8_t>& priv2, const std::vector<uint8_t>& pub2)
{
    ...
    if(ec) std::cout << "  [1] Derive Error: " << ec.message() << std::endl;
    
    // ECDH: 2 的私钥 + 1 的公钥
    auto shared2 = reality::crypto_util::x25519_derive(priv2, pub1, ec);
    if(ec) std::cout << "  [2] Derive Error: " << ec.message() << std::endl;
