...
boost::asio::awaitable<void> socks_udp_session::run(const std::string& host, const uint16_t port)
{
    ...
    const bool completed = co_await finish_udp_session(
        [this]() -> boost::asio::awaitable<bool>
        {
            ...
            {
                co_await (udp_socket_loop() || keep_tcp_alive());
            }
            else
            {
                co_await (udp_socket_loop() || keep_tcp_alive() || idle_watchdog());
            }
            co_return true;
        },
        close_reason_,
    ...
    record_udp_session_close_trace(make_bound_udp_trace_event(trace_id_,
                                                              ...
                                                              "socks",
                                                              proxy_outbound_tag_,
                                                              proxy_outbounds_.empty() ? "direct" : "proxy",
                                                              udp_bind_host_,
                                                              udp_bind_port_,
                                                              tcp_peer_host_,
                                                              tcp_peer_port_,
...
boost::asio::awaitable<void> socks_udp_session::proxy_to_udp_sock(std::string outbound_tag, std::shared_ptr<udp_proxy_outbound> outbound)
{
