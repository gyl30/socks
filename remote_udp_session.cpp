...
asio::awaitable<void> remote_udp_session::watchdog()
{
    while (udp_socket_.is_open())
    {
        ...
        {
            LOG_CTX_WARN(ctx_, "{} read idle {}s", log_event::kTimeout, std::chrono::duration_cast<std::chrono::seconds>(read_elapsed).count());
        }
        if (write_elapsed > std::chrono::seconds(60))
        {
            LOG_CTX_WARN(ctx_, "{} write idle {}s", log_event::kTimeout, std::chrono::duration_cast<std::chrono::seconds>(write_elapsed).count());
        }
    }
